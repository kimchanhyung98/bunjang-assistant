...
- 검색과 상품 조회는 로그인 없이 실행한다. 채팅, 관심상품, 구매 가능 여부 확인은 실행 전 `auth.status`로 로그인 상태를 확인하고, 미로그인이면 실행하지 않고 로그인 필요 상태를 보고한다.
- `auth.login`, 구매 시작, 최종 구매 확정, 계정 설정 변경은 수동 전용이며 래퍼에서 실행하지 않는다.
...
- 인증: `auth.status`, `auth.logout`.
- 수동 전용: `auth.login`, `purchase.start`, `purchase.confirm`, `account.settings.update`.
